...
for i in range(0,totmotes):
	node_meta.append(Nodedetails(0,i,[0 for i in range(history)],[-1 for i in range(history)],[0 for i in range(history)],[[0,0] for i in range(totmotes)],[0,0],[-256 for i in range(totmotes)]))
...
def dataProducer(data,node_ident,param):
	...
	if (param=="tempsense"):
		...
		print string_val
		string_val1=string_val.split('0x')
		if(int(string_val1[2],16) <=15):
			string_val1[2]='0'+string_val1[2]
		hex_string=string_val1[1]+string_val1[2]
		print "Printing hex_string"
		print hex_string
		int_val=int(hex_string,16)
		print "int val"
		print int_val
		Rthr=10000*(1023-int_val)/int_val
		print "Rthr"
		print Rthr
